...
<div class="content">
	...
	<?php
		...
		echo "MODYFIKACJA TESTU: " . $nazwa . "<br>";
		
		$result = mysqli_query($link, "SELECT opis FROM test WHERE idt='$idt'");
		$rekord = mysqli_fetch_array($result);
		$opis = $rekord[0];
		
		$result2 = mysqli_query($link, "SELECT idn FROM test WHERE idt='$idt'");
		$rekord2 = mysqli_fetch_array($result2);
		$idn = $rekord2[0];
		
		echo "<form method='post' action='testmodscript.php?idt=$idt&idn=$idn'>
			Nazwa testu:&nbsp;<input type='text' name='nazwa' value=$nazwa maxlength='50' size='100'><br>
			<br><hr>
			<script type='text/javascript' src='/ckeditor/ckeditor.js'></script>
			<textarea name='opis' id='edytor' value=$opis></textarea>	
			<hr><br>
			
			<input type='submit' value='Modyfikuj test'/>
			</form>
		...
		echo "<br><a href='pytaniatest.php?idt=$idt'>MODYFIKUJ PYTANIA TESTU</a>";
	?>
	</div>
	</div>
</div>
