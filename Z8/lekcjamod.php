...
<div class="content">
	...
	<?php
		...
		echo "MODYFIKACJA LEKCJI: " . $nazwa . "<br>";
		
		$result = mysqli_query($link, "SELECT tresc FROM lekcje WHERE idl='$idl'");
		$rekord = mysqli_fetch_array($result);
		$tresc = $rekord[0];
		
		$result2 = mysqli_query($link, "SELECT idn FROM lekcje WHERE idl='$idl'");
		$rekord2 = mysqli_fetch_array($result2);
		$idn = $rekord2[0];
		
		echo "<form method='post' action='lekcjamodscript.php?idl=$idl&idn=$idn' enctype='multipart/form-data'>
			Nazwa lekcji:&nbsp;<input type='text' name='nazwa' value='$nazwa' maxlength='50' size='100'><br>
			<br><hr>
			<script type='text/javascript' src='/ckeditor/ckeditor.js'></script>
			<textarea name='tekst' id='edytor' value=$tresc></textarea>	
			<hr><br>
			
			Plik do lekcji: <input type='file' name='file' id='file'><br><br>
			<input type='submit' value='Modyfikuj lekcję'/>
			</form>
